...
inline u64 hton64(u64 v){
    ...
    return (((u64)htonl((uint32_t)(v & 0xffffffffULL)))<<32) |
            htonl((uint32_t)((v>>32) & 0xffffffffULL));
#else 
    return v;
#endif
}
...
#if __BYTE_ORDER == __LITTLE_ENDIAN
    return (((u64)ntohl((uint32_t)(v & 0xffffffffULL)))<<32) |
            ntohl((uint32_t)((v>>32) & 0xffffffffULL));
#else 
    return v;
#endif
}
...
public:
    pair<string,u64> handle(u64 sess,const string &cmdline){
        ...
        if(cmd=="logout"){
            if(!sessions.count(sess)||sessions[sess].empty())
                return {"ERR not logged in",sess};
            sessions[sess].clear();
...
// ------------------- Forwarding queue (Tracker1 only) -------------------
